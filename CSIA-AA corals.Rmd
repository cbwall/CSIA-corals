...
ggplot(df.mean, aes(x=AA.short, y=d13C.value), position=pd) +
...
pd <- position_dodge(0.5) #offset for error bars
ggplot(AA.means, aes(x=Treat.Int, y=d13C.value, shape=AA.cat, color=Fraction)) +
  geom_errorbar(aes(ymin=d13C.value-SD, ymax=d13C.value+SD, shape=AA.cat, color=Fraction), size=.5, width=0, position=pd) +
...
pd <- position_dodge(0.4) #offset for error bars
...
ggplot(sumVdf, aes(x=Treat.Int, y=mean.sumV)) +
  geom_point(aes(color=Fraction), size=3, position=pd) +
...
<center>
![](figures/PCAs_compiled.png){ width=100%} </center>
