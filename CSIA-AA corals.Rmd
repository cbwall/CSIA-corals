...
df.SD.frac<-aggregate(d13C.value~AA.short+Fraction, data=dfC, sd, na.rm=TRUE)
colnames(df.SD.frac)[3]="SD"
df.mean.frac<-cbind(df.mean.frac, df.SD.frac[3])
...
df.SD3<-aggregate(d13C.value~AA.short+Fraction, data=dfC, na.rm=TRUE, sd)
df.SD3[is.na(df.SD3)] <- 0
colnames(df.SD3)[3]="SD"
df.mean3<-cbind(df.mean3, df.SD3[3])
...
df.mean$Fraction<-factor(df.mean$Fraction, levels=c("host", "symb", "plank"))
write.csv(df.mean, "output/d15N.frac.meansd.csv")
