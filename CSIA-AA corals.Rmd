...
![](figures/iso.sources.KBay.png){ width=40%}  
...
bulk.d15N.means<-aggregate(d15N~Fraction+Treat.Int, data=bulk, mean, na.rm=TRUE)
bulk.d13C.means<-aggregate(d13C~Fraction+Treat.Int, data=bulk, mean, na.rm=TRUE)
bulk.CN.means<-aggregate(C.N~Fraction+Treat.Int, data=bulk, mean, na.rm=TRUE)
...
(1) **Bulk δ^13^C: Fraction and Treatment**
```{r, bulk C isotopes}
anova(lm(d13C ~ Fraction+Treat.Int, data=bulk.HS))
```
  
(2) **Bulk δ^15^N: Fraction and Treatment**
```{r, bulk N isotopes}
# d15N
...
(3) **Bulk coral-symbiont C:N: Fraction and Treatment**
```{r, bulk C:N isotopes}
####
# C:N
...
(4) **Bulk difference in coral-symbiont δ^13^C: Treatment**
```{r, bulk dHS C isotopes}
####
# d13H-S
d13C.HS.plot<-ggplot(bulk.HS, aes(y=d13C.H.S, x=Treat.Int), na.rm=T)+
  geom_boxplot(fill=c("lightblue", "goldenrod1", "gray50")) + 
  geom_dotplot(fill=c("lightblue", "lightblue", "darkgoldenrod1",  "darkgoldenrod1", "gray50", "gray50"),
               binaxis='y', stackdir='center', dotsize = 0.5, alpha=0.5,
               position = position_dodge(0.75))+
...
(5) **Bulk differences in coral-symbiont δ^15^N: Treatment**
```{r, bulk dHS N isotopes}

# d15NH-S
d15N.HS.plot<-ggplot(bulk.HS, aes(y=d15N.H.S, x=Treat.Int), na.rm=T)+
  geom_boxplot(fill=c("lightblue", "darkgoldenrod1", "gray50")) + 
  geom_dotplot(fill=c("lightblue", "lightblue", "darkgoldenrod1",  "darkgoldenrod1", "gray50", "gray50"),
               binaxis='y', stackdir='center', dotsize = 0.5, alpha=0.5,
               position = position_dodge(0.75))+
...
(6) **Bulk tissue isotope coral-symbiont figure**
```{r, bulk C and N isotopes, fig.dim=c(8.5,2.5)}
####
...
bulk.figures<-(d13C.plot+theme(legend.position = "none") | d15N.plot | C.N.plot | d13C.HS.plot | d15N.HS.plot |
                 bulk.legend)
print(bulk.figures)
ggsave("figures/execute code/Fig 1. bulk isotope.pdf", height=3.5, width=12, encod="MacRoman")
```
...
#### Permanova  
Carbon isotope PERMANOVA using levels of Fraction and Treatment
```{r, Carbon PERMANOVA}
## Permanova carbon
...
Nitrogen isotope PERMANOVA using levels of Fraction and Treatment
```{r, Nitrogen PERMANOVA}
## Permanova nitrogen
...
#### PCA 
Four PCAs  
(a) *Carbon PCA by fraction (plankton, host, symbiont)*  
(b) *Nitrogen PCA by fraction (plankton, host, symbiont)*    
(c) *Carbon PCA by treatment*  
(d) *Nitrogen PCA by treatment*  
...
#ggsave("figures/execute code/carbon/PCA_d13C.frac.pdf", height=5, width=8, encod="MacRoman")
```
...
#ggsave("figures/execute code/nitrogen/PCA_d15N.frac.pdf", height=5, width=8, encod="MacRoman")
```
```{r PCA carbon by treatment, results='hide', message=FALSE, fig.show='hide'}
######## ######## ######## ######## carbon by treatment
# PCA dataframe
PCA.df.C$Treat.Int<-factor(PCA.df.C$Treat.Int,
                              levels=c("L-NF", "L-F", "D-F", "plank"))
...
PC.fig1.C.trt <- ggbiplot(PC.C, choices = 1:2, obs.scale = 1, var.scale = 1, 
  ...
  annotate("text", x=6, y=3.6,  size=5, label= expression(paste(delta^{13},C[AA])))
print(PC.fig1.C.trt)
#ggsave("figures/execute code/carbon/PCA_d13C.trt.pdf", height=5, width=8, encod="MacRoman")
```
```{r PCA nitrogen by treatment, results='hide', message=FALSE, fig.show='hide'}
######################## nitrogen by treatments
PCA.df.N$Treat.Int<-factor(PCA.df.N$Treat.Int,
                              levels=c("L-NF", "L-F", "D-F", "plank"))
...
PC.fig2.N.trt <- ggbiplot(PC.N, choices = 1:2, obs.scale = 1, var.scale = 1, 
  ...
  annotate("text", x=4, y=5.5,  size=5, label= expression(paste(delta^{15},N[AA])))
print(PC.fig2.N.trt)
#ggsave("figures/execute code/nitrogen/PCA_d15N.trt.pdf", height=5, width=6, encod="MacRoman")
```
...
ggsave("figures/execute code/Fig 2.PCAs.pdf", height=8, width=11, encod="MacRoman")
...
Make a table and figure for the d13C by fraction and treatments
...
#ggsave("figures/execute code/carbon/d13C.CSIA_frac.trt.pdf", height=5, width=8, encod="MacRoman")
...
#ggsave("figures/execute code/carbon/d13C.CSIA_Trt.alone.pdf", height=5, width=8, encod="MacRoman")
...
#ggsave("figures/execute code/carbon/d13C.CSIA_Fraction.pdf", height=5, width=8, encod="MacRoman")
```
...
```{r, fig.dim=c(6,3), fig.show='hide', results='hide'}
...
Code chunks here for figures...  
(1) *all AA pooled by fraction, showing d15N.CSIA_Fraction*  
(2) *fraction and treatment, showing d15N.CSIA_frac.trt*  
(3) *all amino acids together*  
```{r, results='hide'}
######## Figures
pd <- position_dodge(0.5) #offset for error bars
...
#ggsave("figures/execute code/nitrogen/d15N.CSIA_Fraction.pdf", height=5, width=8, encod="MacRoman")
```
```{r, results='hide'}
#################
...
#ggsave("figures/execute code/nitrogen/d15N.CSIA_frac.trt.pdf", height=5, width=8, encod="MacRoman")
```
```{r, results='hide'}
##################
...
#ggsave("figures/execute code/nitrogen/d15N.CSIA_Trt.alone.pdf", height=5, width=8, encod="MacRoman")
```
...
dev.copy(pdf, "figures/execute code/Fig 3.AA.frac.trt.pdf", width = 10, height = 5, encod="MacRoman")
dev.off()
```  
...
dev.copy(pdf, "figures/execute code/Fig S1ab.AA.frac.pdf", width = 10, height = 5, encod="MacRoman")
dev.off()
```  
...
dev.copy(pdf, "figures/execute code/Fig S1cd.AA.trt.pdf", width = 10, height = 5, encod="MacRoman")
dev.off()
```  
...
Where beta = 3.4 and TDF (trophic discrimination factor) is 7.6
```{r, results='hide'}
###
...
TP<-ggplot(df.mean, aes(x=Treat.Int, y=TP)) +
  ...
  Fig.formatting
#print(TP)
#ggsave("figures/execute code/nitrogen/TP.glx.phe.pdf", height=5, width=8, encod="MacRoman")
```
...
```{r, results='hide', warning=FALSE, fig.dim=c(4,4)}
###########
###########
# d15N.CSIA.dat long
d15N.dat.long<-read.csv("data/d15N.CSIA.long.csv")
...
sumV<-ggplot(sumVdf, aes(x=Treat.Int, y=mean.sumV)) +
  ...
  Fig.formatting
#print(sumV)
#ggsave("figures/execute code/nitrogen/d15N.sumV.CSIA.pdf", height=5, width=8, encod="MacRoman")
```
...
```{r, results='hide', warning=FALSE, fig.cap="*Figure S3*. δ^15^N~AA~ weighted means for trophic and source amino acids according to coral tissue fractions (host, symbiont) and experimental nutrition-treatments. Values are mean ± SD (n=2).", fig.dim=c(4,4)}
###########################
##################
#########
...
ggplot(wt.mean.df, aes(x=Treat.Int, y=wt.mean)) +
  ...
  ylab(expression(paste(delta^{15}, N[AA], " (\u2030, Air)"))) +
  Fig.formatting
ggsave("figures/execute code/Fig S3.wt.mean.d15N.CSIA.pdf", height=5, width=8, encod="MacRoman")
```
...
dev.copy(pdf, "figures/execute code/Fig S2.TP.sumV.pdf", width = 7, height = 5, encod="MacRoman")
dev.off()
```  
...
Combine the data from Oahu and Palmyra and run LDA analysis as performed in Fox et al.   
```{r, Palmyra data}
##### Palmyra data
...
EAA.frac.LD.norm.meta$Location<-revalue(EAA.frac.LD.norm.meta$Location,
                                       c("Oahu"="Hawaii"))
...
ggsave("figures/execute code/Fig 4b. LDA.PacRedSea.pdf", height=5, width=5)
##############################################################
...
```{r Pac means, fig.cap="*Figure S4*. Comparison of δ13C values for non-essential and essential amino acids in a pooled plankton sample from Kāne‘ohe Bay, O‘ahu, Hawai‘i and plankton and POM from Palmyra (reported in (Fox et al. 2019)). Values are mean ± SE (n=6-9), except for the Hawai‘i plankton (n=1).", fig.dim=c(6,6)}
...
ggsave("figures/execute code/Fig S4. Pacific.plank.means.pdf", height=5, width=6, encod="MacRoman")
```
...
write.csv(Algae_plank.ess.norm, "output/Algae_plank.ess.norm.csv")
##############################################################
######## PCA
##############################################################
...
#PCA plot
#print(PCA.plank)
#ggsave("figures/execute code/Fig S5a. PCA.EAAn.plank.pdf", height=5, width=5)
...
#PCA plot
#print(PCA.plank.sym)
#ggsave("figures/execute code/Fig S5b. PCA.EAAn.plan.alg.symb.pdf", height=5, width=5)
...
#PCA plot
print(PCAs.supp)
ggsave("figures/execute code/Fig S5comb. PCA.EAAn.plan.alg.symb.pdf", height=5, width=9)
